...
create policy "authors or admins can delete books"
...
-- Chapters inherit visibility from their book
create policy "chapters of published books are public"
  on public.book_chapters for select
  using (exists (select 1 from public.books b where b.id = book_id and b.status = 'published'));
